...
__global__ void SOR_add (float* x)
{
        ...
        int col_start = threadIdx.x + blockIdx.x * blockDim.x;
        int row_start = threadIdx.y + blockIdx.y * blockDim.y;

        int i;
        float result = 0;

        int offset = col_start + row_start*blockDim.x*gridDim.x;
	

         if((offset > 2047)  && (offset%2048) && (offset%2047) && (offset < SM_ARR_LEN-2047))
	{

        	result = x[offset] - 0.25*(x[offset-1] + x[offset+1] + x[offset - blockDim.x*gridDim.x] + x[offset + blockDim.x*gridDim.x]);
        	__syncthreads();
