...
main(int argc, char *argv[])
{
  ...
  void ArrayAdd(data_t* pArray1, data_t* pArray2, data_t* pResult, long int nSize);
  void ArrayMul(data_t* pArray1, data_t* pArray2, data_t* pResult, long int nSize);
  void ArrayDot(data_t* pArray1, data_t* pArray2, data_t* result, long int nSize);
  ...
  dot_data = (data_t *) malloc(sizeof(data_t));
  ...
  for (i = 0; i < ITERS; i++) {
    clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &time1);
    ArrayAdd(pArray1, pArray2, pResult, BASE+(i+1)*DELTA);
    clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &time2);
    time_stamp[OPTION][i] = diff(time1,time2);
  }
  ...
  for (i = 0; i < ITERS; i++) {
    clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &time1);
    ArrayMul(pArray1, pArray2, pResult, BASE+(i+1)*DELTA);
    clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &time2);
    time_stamp[OPTION][i] = diff(time1,time2);
  }
  ...
  for (i = 0; i < ITERS; i++) {
    clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &time1);
    ArrayDot(pArray1, pArray2, dot_data, BASE+(i+1)*DELTA);
    clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &time2);
    time_stamp[OPTION][i] = diff(time1,time2);
  }
...
/*Element wise addition with SSE intrinsics*/
void ArrayAdd(data_t* pArray1, data_t* pArray2, data_t* pResult, long int nSize)
{
    int i, nLoop = nSize/4;
...
/*Element wise multiplication with SSE intrinsics*/
void ArrayMul(data_t* pArray1, data_t* pArray2, data_t* pResult, long int nSize)
{
    int i, nLoop = nSize/4;
...
/*Dot product with SSE Intrinsics*/
void ArrayDot(data_t* pArray1, data_t* pArray2, data_t* result, long int nSize)
{
    int i, nLoop = nSize/4;
