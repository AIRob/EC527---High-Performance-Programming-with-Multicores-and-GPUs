...
main(int argc, char *argv[])
{
  ...
  void transpose(vec_ptr v,data_t *dest);
  void transpose_rev(vec_ptr v,data_t *dest);
  ...
  data_holder = (data_t *) calloc((MAXSIZE*MAXSIZE),sizeof(data_t));
...
for (i = 0; i < length; i++)
	for (j = 0; j < length; j++) 
		dest[(j*length)+i] = data[(i*length)+j];
//free(dest);
}
...
for (i = 0; i < length; i++)
	for (j = 0; j < length; j++) 
		dest[(i*length)+j] = data[(j*length)+i];
//free(dest);
}
